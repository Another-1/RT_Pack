    ...
    if ( $nul -ne $settings.telegram.tg_token -and '' -ne $settings.telegram.tg_token -and $settings.telegram.report_obsolete -and $settings.telegram.report_obsolete -eq 'Y' ) {
        ...
        if ( $obsolete_torrents.count -gt 0 ) {
            $obsolete_torrents | ForEach-Object {
                ...
                $obsolete_status = $res.total_count -gt 0 ? (Get-StatusTitles)[$res.releases[ $res.releases.count - 1 ][$res.columns.indexof('tor_status')].ToInt32($nul)] : 'разрег'
                    ...
                    if ( !$id_to_info[$rss_record[1].ToInt32($null)] -and $rss_record[3] -notin $clients_torrents.hash ) {
                        if ( !$ignored -or $requester -notin $ignored ) {
                            ...
                            if ( !$fresh_hash ) {
                                ...
                                if ( $fresh_status -notin @( 'не оформлено', 'премодерация', 'повтор', 'закрыто' ) ) {
...
if ( $report_stalled -eq 'Y' ) {
    ...
    if ( $stalleds.count -gt 0 ) {
        ...
        foreach ( $stalled in $stalleds ) {
            Write-Log "Смотрим статус трекера по некачашке $($stalled.hash) $($stalled.topic_id)"
            $params = @{
                hash = $stalled.hash
            }
