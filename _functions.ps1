...
function Initialize-Client ( $client, $mess_sender = '', [switch]$verbose, [switch]$force ) {
    if ( !$client.sid -or $force ) {
        ...
        try {
            ...
            if ( $tg_token -ne '' ) {
                Send-TGMessage "Нет связи с клиентом $( $client.Name ) при вызыве из $( (Get-PSCallStack)[(( Get-PSCallStack ).count - 2)..0].Command | Where-Object { $null -ne $_ } | Join-String -Separator ' → ' ). Процесс остановлен." $tg_token $tg_chat $mess_sender
            }
            Exit
        }
    }
